...
usuario_cadastrados="DadosLogin.txt"
...
def editar_produtos(id_velho, n_id, n_nome, n_preco):
    ...
    try:
        ...
        for linha in linhas:
            ...
            if len(dados) >= 3 and (dados[1].upper() == id_velho ):
                n_preco = n_preco.strip().replace(',', '.')
...
def deletar_usuario(loginusuario):
    with open("DadosLogin.txt", "w", encoding="utf-8") as f:
        for linha in f:
            linhas = linha.strip(",")
            if loginusuario not in linhas:
                return f"✅ {loginusuario} Usuário nao encontrado!"
            usuario_fora=linhas.splir(",")
            if usuario_fora[0] == loginusuario:
                

                f.write(linha)
    return f"✅ {linha}Usuário deletado com sucesso!"
